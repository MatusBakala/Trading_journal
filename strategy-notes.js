...
async function seedDefaultStrategies(){
  ...
  // Inštalácie zo staršej verzie appky už majú seedovanie odbavené, takže si
  // nevšimnú nové položky pribudnuté do DEFAULT_STRATEGIES odvtedy – domiešaj
  // len tie, čo tu chýbajú podľa mena, existujúce stratégie nechaj netknuté.
...
function openStrategyDetail(id){strategyDetailId=id;strategyDetailTab='rules';strategyNotesEdit=false;strategyRulesEdit=false;strategyScenariosEdit=false;renderStrategies();}
function closeStrategyDetail(){strategyDetailId=null;renderStrategies();}
function switchStrategyDetailTab(tab){strategyDetailTab=tab;strategyNotesEdit=false;strategyRulesEdit=false;strategyScenariosEdit=false;renderStrategies();}
...
const RT_DEFAULT_FONT='Helvetica';
...
function toggleStrategyScenariosEdit(){strategyScenariosEdit=!strategyScenariosEdit;renderStrategies();}
function scenarioEditRowHTML(strategyId,sc){
  sc=sc||{tradeId:null,note:''};
  const options=trades.filter(t=>t.strategyId===strategyId).sort((a,b)=>tTime(b)-tTime(a)).map(t=>{
...
function ensureScenarioShots(tradeId){
  ...
  shotsByTrade(tradeId).then(shots=>{
    ...
    if(strategyDetailId!=null&&strategyDetailTab==='scenarios'&&!strategyScenariosEdit)renderStrategies();
  });
}
function showLightbox(url){$('lightboxImg').src=url;$('lightbox').classList.add('open');}
function scenarioViewCardHTML(sc){
  const t=trades.find(x=>x.id===sc.tradeId);
...
function renderStrategyDetail(box){
  ...
  const tabBtns=tabs.map(([k,label])=>`<button class="btn ${strategyDetailTab===k?'':'secondary'} small" data-action="switchStrategyDetailTab" data-tab="${k}">${esc(label)}</button>`).join('');
  let body;
  ...
  }else if(strategyDetailTab==='scenarios'){
    if(strategyScenariosEdit){
      const noTrades=!trades.some(t=>t.strategyId===s.id);
  ...
  }else if(strategyDetailTab==='trades'){
...
function goToTradesForHour(hour){
  goToTab('trades');
  const sel=$('fHour');
