...
function shrinkImageBlob(blob,maxDim){
...
function buildTradeReviewData(t){
...
async function aiReviewTrade(){
  ...
  const content=[];
  try{
    const shots=currentTradeId!=null?await shotsByTrade(currentTradeId):[];
  ...
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{
      ...
      body:JSON.stringify({
        model:settings.aiChatModel||'claude-sonnet-5',
        max_tokens:1600,
    ...
    const review={text,at:Date.now(),model:settings.aiChatModel||'claude-sonnet-5',images:content.length-1};
    renderAiReview(review);
    if(currentTradeId!=null){ // ulož, nech sa nemusí (a neplatí) generovať znova
      const stored=trades.find(x=>x.id===currentTradeId);
...
function renderAiReview(review){
  ...
  const when=new Date(review.at).toLocaleString(settings.lang==='en'?'en-GB':'sk-SK');
...
function openInTradingView(){
  const t=formTrade();
  if(!t.symbol){toast('Zadaj symbol');return;}
  const iv={'1m':'1','5m':'5','15m':'15','30m':'30'}[modalChartTf]||'5';
...
function renderModalChart(){
  ...
  try{modalChart.timeScale().setVisibleRange({from:tzOff(t1-padView),to:tzOff(t2+padView)});}
  ...
  if(modalIndicators.rsi){
    ...
    modalChartRsi.timeScale().setVisibleRange({from:tzOff(t1-padView),to:tzOff(t2+padView)});
