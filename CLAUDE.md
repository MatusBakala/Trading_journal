...
6. Existujúci flag `defaultStrategiesSeeded` v `kv` store zabezpečuje, že sa seedovanie nespustí znova na už použitých inštaláciách (vrátane tejto testovacej) – po zmene DEFAULT_STRATEGIES treba na už-seedovanej inštancii ručne overiť/aktualizovať dáta, seedovanie sa samo nespustí znova.